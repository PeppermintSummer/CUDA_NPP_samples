...
{
	//将input_point，进行插值，得到像素值，并赋值给output_image[output_point]
	...
	//将input_point进行插值，并且赋值给output_image[output_point]
	// 如果input_point对应到原图时越界了，则这个点不需要考虑
...
void warpAffine()
{
	...
	cv::warpAffine(image, output, M, affine.size(), 1, 0, Scalar::all(114)); //边界颜色
	imwrite("output-cpu.jpg", output);//

	//GPU
	auto gpu_M = get_gpu_affine_matrix(image.size(), affine.size()); //	求得是 逆矩阵
